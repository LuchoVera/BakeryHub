...
public class ProductService : IProductService
{
    ...
    {
        var product = await _productRepository.GetByIdForAdminAsync(productId, adminTenantId);
    ...
    public async Task<bool> UpdateProductForAdminAsync(Guid productId, UpdateProductDto productDto, Guid adminTenantId)
    {
        ...
        var newTagNames = productDto.Tags?.Where(t => !string.IsNullOrWhiteSpace(t))
        ...
        product.ProductTags
               ...
               .ToList()
               .ForEach(pt => product.ProductTags.Remove(pt));

        var currentTagNames = product.ProductTags.Select(pt => pt.Tag.Name).ToList();
        foreach (var tagName in newTagNames)
        {
            if (!currentTagNames.Contains(tagName, StringComparer.OrdinalIgnoreCase))
            {
    ...
    {
        var productToSoftDelete = await _productRepository.GetByIdAsync(productId);
        if (productToSoftDelete == null || productToSoftDelete.TenantId != adminTenantId) return false;

        bool isInActiveOrder = await _orderChecker.IsProductInActiveOrderAsync(productId);
        if (isInActiveOrder)
        {
            throw new InvalidOperationException("This product cannot be deleted...");
        }
    ...
    {
        var product = await _productRepository.GetByIdPublicAsync(productId, tenantId);
        if (product == null) return null;
        return await MapProductToDtoAsync(product);
    }
